...
BEGIN
  IF EXISTS (
    ...
    WHERE polname = 'Users can manage their own achievements' 
    AND polrelid = 'public.user_achievements'::regclass
  ) THEN
    DROP POLICY "Users can manage their own achievements" ON public.user_achievements;
  END IF;
END $$;

-- Create RLS policy (without IF NOT EXISTS as it's not supported)
